...
> Each changelog item represents a user-facing announcement and may summarize multiple pull requests from a single release.
...
- **Upload location:** Images should be uploaded to the `public-flavor-logos` S3 bucket in the `whats_new/` folder. Use your `default` AWS profile (not an assumed role):
  ```bash
  aws s3 cp /path/to/image.png \
    s3://public-flavor-logos/whats_new/your-image.png \
    --profile default
  ```
...
6. **Grouping:** Automated changelog entries generated by our tooling typically represent grouped themes (2-3 per release) rather than individual PRs.
