...
def build_release_notes_body_prompt(
    ...
    return (
        ...
        "- Write one bullet per distinct user-facing change, not per PR. A PR that does a single thing "
        "(e.g. fixes one bug) gets a single bullet; a PR that genuinely does several separate things may get "
        "one bullet per thing. Never pad a single change into multiple bullets (e.g. a separate 'what "
        "changed' bullet and a 'why it matters' bullet), and never restate the same change as both a framing "
        "sentence and a bullet.\n"
        "- Start each bullet with a short bold lead-in naming the change, then 2-3 sentences explaining "
        "what changed, why it matters, and what users can now do. Give enough detail to be genuinely "
        "useful, not just a one-line summary. "
        "Example: `- **Faster run queries**: Common pipeline and step run views now load more efficiently on "
        "large deployments. ZenML adjusted how related data is fetched, avoiding expensive query plans that "
        "previously slowed down paginated run listings.`\n"
        "- Only add a `#### Subsection` header when two or more PRs share a clear theme, and group those PRs "
        "as bullets under it. When PRs do not share a theme, list them as flat top-level bullets with no "
        "subsection header and no framing sentence.\n"
        "- Collect all bug-fix bullets together inside a single `<details><summary>Fixed</summary>...</details>` "
        "block rather than one block per fix.\n"
